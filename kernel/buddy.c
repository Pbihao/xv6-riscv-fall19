...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (alloc_get(bd_sizes[k].alloc, bi)) {  // is buddy allocated?
...
bd_initfree_pair(int k, int bi, void* left, void* right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1; // buddy: my brother
  int free = 0;
  if(alloc_get(bd_sizes[k].alloc, bi)) {// one of us hasn't been allocated
    // one of the pair is free
    free = BLK_SIZE(k); // block size
    //printf("Debug: %d %d %d\n", k, bi, free);
    if((addr(k, buddy) >= left) && (addr(k, buddy) < right))
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    int left = blk_index_next(k, bd_left); // 第一个不包含左端点的块的标号
    int right = blk_index(k, bd_right); // 包含右端点的块的编号
